...
public class BuyingHistory extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<PurchaseHistory> list = o.getPurchaseHistory(user.ID);
        int endPage = list.size() / 10;
        if (list.size() % 10 != 0) {
            endPage++;
        }
        int startP = (idx - 1) * 10;
        List<PurchaseHistory> listp = new ArrayList<>();
        for (int i = startP; i < startP + 10 && i < list.size(); i++) {
